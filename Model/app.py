...
# =========================
async def serve_protected_page(file_path: str, request: Request, db: AsyncSession):
...
async def serve_index(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if email:
        ...
        user = result.scalar_one_or_none()
        if user:
            return RedirectResponse("/admin" if user.role == "Admin" else "/user")
    with open(INDEX_HTML_PATH, "r") as f:
        return HTMLResponse(content=f.read())

@app.get("/admin", response_class=HTMLResponse)
async def serve_admin(request: Request, db: AsyncSession = Depends(get_db)):
    return await serve_protected_page(os.path.join(FRONTEND_DIR, "admin", "admin.html"), request, db)

@app.get("/user", response_class=HTMLResponse)
async def serve_user(request: Request, db: AsyncSession = Depends(get_db)):
    return await serve_protected_page(USER_HTML_PATH, request, db)

@app.get("/guide", response_class=HTMLResponse)
async def serve_guide(request: Request, db: AsyncSession = Depends(get_db)):
    return await serve_protected_page(os.path.join(FRONTEND_DIR, "user", "guide.html"), request, db)

@app.get("/comic", response_class=HTMLResponse)
async def serve_comic(request: Request, db: AsyncSession = Depends(get_db)):
    return await serve_protected_page(os.path.join(FRONTEND_DIR, "user", "comic.html"), request, db)

@app.get("/history", response_class=HTMLResponse)
async def serve_history(request: Request, db: AsyncSession = Depends(get_db)):
    return await serve_protected_page(os.path.join(FRONTEND_DIR, "user", "history.html"), request, db)

@app.get("/settings", response_class=HTMLResponse)
async def serve_settings(request: Request, db: AsyncSession = Depends(get_db)):
    return await serve_protected_page(os.path.join(FRONTEND_DIR, "user", "settings.html"), request, db)
...
async def login(user_data: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    response = JSONResponse(content={"message": "Login successful", "redirect_url": "/admin" if user.role == "Admin" else "/user"})
    response.set_cookie(key="access_token", value=token, httponly=True, samesite="Lax")
    return response
